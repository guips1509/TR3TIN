...
<html>
    ...
    <body>
     ...
     <div class="page-header">
        <h1 style="color:white">Poissons eau de mer</h1>
      </div>
        <div class="conteneur">
        ...
        <div id="menu_articles"> 
            ...
            <form action="Produits2.php" method="post">
        ...
        if(!empty($_GET['ordre']))
            $_POST['ordre'] = $_GET['ordre'];
                     ...
                     {
                         $rep = $bdd->query('SELECT * FROM poissonsdemer ORDER BY nom_commun LIMIT ' . ($nbreArticles * $pageActuelle) . ', ' . $nbreArticles);
                     ...
                     {
                         $rep = $bdd->query('SELECT * FROM poissonsdemer ORDER BY nom_commun DESC LIMIT ' . ($nbreArticles * $pageActuelle) . ', ' . $nbreArticles);
                        while($donnees=$rep->fetch())
                        {
                             echo '<div class="article" ><p>'.$donnees['nom_commun'].'<br/>
                                 '.$donnees['taille'].' cm<br/>
                         ...
                         {
                         $rep = $bdd->query('SELECT * FROM poissonsdemer ORDER BY prix DESC LIMIT ' . ($nbreArticles * $pageActuelle) . ', ' . $nbreArticles);
                        while($donnees=$rep->fetch())
                        {
                             echo '<div class="article" ><p>'.$donnees['nom_commun'].'<br/>
                                 '.$donnees['taille'].' cm<br/>
                         ...
                         {
                         $rep = $bdd->query('SELECT * FROM poissonsdemer ORDER BY prix LIMIT ' . ($nbreArticles * $pageActuelle) . ', ' . $nbreArticles);
                        while($donnees=$rep->fetch())
                        {
                             echo '<div class="article" ><p>'.$donnees['nom_commun'].'<br/>
                                 '.$donnees['taille'].' cm<br/>
        ...
        <?php
            ...
            $req_count=$bdd->query('SELECT COUNT(*) AS resultat FROM poissonsdemer');
            ...
            for($i=0;$i<$nbPages;$i++)
            {
                ...
                if(isset($_POST['ordre']))
                    echo '<li ' . $active. '><a href="Produits2.php?page=' . $i . '&ordre=' . $_POST['ordre'] . '&nbreArticles='. $nbreArticles . '">'. ($i+1) .'</a></li>';
                else
                    echo '<li ' . $active. '><a href="Produits2.php?page=' . $i . '">'. ($i+1) .'</a></li>';
                    
            }
                if(isset($_POST['ordre']))
                    echo '<li><a href="Produits2.php?page=' . ($pageActuelle+1) . '&ordre=' . $_POST['ordre'] .' ">&raquo;</a></li></ul>';
                else
                    echo '<li><a href="Produits2.php?page=' . ($pageActuelle+1) .' ">&raquo;</a></li></ul>';
              echo'</div>';  
           ?> 
    </body>
